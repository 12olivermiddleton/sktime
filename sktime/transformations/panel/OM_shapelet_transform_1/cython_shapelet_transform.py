...
class RandomShapeletTransform(_PanelToTabularTransformer):
    ...
    def transform(self, X, y=None):
        ...
        for i, series in enumerate(X):
            dists = Parallel(n_jobs=self._n_jobs)(
                delayed(utils.online_shapelet_distance)(
                    series[shapelet[3]],
                    shapelet[6],
    ...
    def _find_shapelet_quality(
        ...
        for i, series in enumerate(X):
            if i != inst_idx:
                distance = utils.online_shapelet_distance(
            ...
            if worst_quality > 0:
                quality = utils.calc_early_binary_ig(
                    orderline,
                    this_cls_traversed,
                    other_cls_traversed,
        ...
        quality = utils.calc_binary_ig(orderline, this_cls_count, other_cls_count)
    ...
    def _remove_self_similar_shapelets(shapelet_heap):
        ...
        for i in range(len(shapelet_heap)):
            ...
            for n in range(i + 1, len(shapelet_heap)):
                if to_keep[n] and utils.is_self_similar(shapelet_heap[i], shapelet_heap[n]):
